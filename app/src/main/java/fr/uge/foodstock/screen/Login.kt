...
private fun loginApi(queue: RequestQueue, params: Pair<String, String>, onSuccessCallback: (String) -> Unit, onErrorCallback: (String) -> Unit) {
    val urlToCall = ScannerBarCode.apiUrl + "login"
    val request = object : StringRequest(
        Method.POST, urlToCall, { response ->
            ...
            error?.networkResponse?.statusCode?.let {
                if (it == 403) {
                    onErrorCallback("Don't find account")
                } else {
            ...
            Button(
                onClick = {
                    ...
                    loginApi(
                        ...
                        { token ->
                            Log.i("Login", "the token is $token")
                            navController.navigate(Routes.Scanner.route)
                            onLoginSuccess(username, password, token)
                        }, {
                            error.value = it
                        }
                    )
